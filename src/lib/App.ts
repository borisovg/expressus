...
type MiddlewareFunction<T1 extends Request, T2 extends Response> = (req: T1, Response: T2, next: MiddlewareCallbackFunction) => Promise<void> | void;
type HandlerFunction<T1 extends Request, T2 extends Response> = (req: T1, res: T2) => Promise<void> | void;
type RequestParams = Record<string, string>;
type RequestSplat = string | null;
type RouteHandlerFunction<T1 extends Request, T2 extends Response> = (req: T1, res: T2, params: RequestParams, splat: RequestSplat) => Promise<void> | void;
...
function make_handler<T1 extends Request, T2 extends Response>(callback: HandlerFunction<T1, T2>): RouteHandlerFunction<T1, T2> {
    return function handler(req: T1, res: T2, params: RequestParams, splat: RequestSplat) {
        req.params = params;
        req.splat = splat;
...
class App {
    ...
    delete<T1 extends Request, T2 extends Response>(path: string, handler: HandlerFunction<T1, T2>) {
        this._routes.DELETE.set(path, make_handler(handler));
    }

    get<T1 extends Request, T2 extends Response>(path: string, handler: HandlerFunction<T1, T2>) {
        this._routes.GET.set(path, make_handler(handler));
    }

    options<T1 extends Request, T2 extends Response>(path: string, handler: HandlerFunction<T1, T2>) {
        this._routes.OPTIONS.set(path, make_handler(handler));
    }

    patch<T1 extends Request, T2 extends Response>(path: string, handler: HandlerFunction<T1, T2>) {
        this._routes.PATCH.set(path, make_handler(handler));
    }

    post<T1 extends Request, T2 extends Response>(path: string, handler: HandlerFunction<T1, T2>) {
        this._routes.POST.set(path, make_handler(handler));
    }

    put<T1 extends Request, T2 extends Response>(path: string, handler: HandlerFunction<T1, T2>) {
